...
#define ALINK_TOPIC_PROP_POST_TURANGSHIDU      "/a1946HaV1CI/STM32/user/turangshidu"
...
#define ALINK_TOPIC_PROP_POST_PAIZHAO    "/a1946HaV1CI/STM32/user/paizhao"
...
void mqttCheckConnect()
{
    while (!client.connected())
    {
      if(client.connect(CLIENT_ID, MQTT_USRNAME, MQTT_PASSWD)){
        ...
        client.subscribe(ALINK_TOPIC_PROP_POST_PAIZHAO);
      }
      delay(10);
    }
}
...
void read_data()
{
  ...
  if(USART_RX_STA&0x8000){
    ...
    else if(msg_data.substring(0, 13)=="Soil moisture"){
      int Soil_moisture=msg_data.substring(13).toInt();
      String new_data="{\"id\":\"ESP8266\",\"version\":\"1.0\",\"params\":{\"Soil moisture\":wildcard_soil}}";
      new_data.replace("wildcard_soil",String(Soil_moisture)); 
      char* charPtr = new char[new_data.length() + 1];
      new_data.toCharArray(charPtr,new_data.length() + 1);
      client.publish(ALINK_TOPIC_PROP_POST_TURANGSHIDU,charPtr);
      free(charPtr);
      ESP.wdtFeed();
    }
    msg_data="";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  //Serial.println();
    ...
    // 获取params中的字段值
    String dengstr = doc["params"]["deng"];
    int deng = dengstr.substring(7).toInt();
    //Serial.println( dengstr.substring(7));
    String fengshanstr = doc["params"]["fengshan"];
    int fengshan = fengshanstr.toInt();
    
    int ding = doc["params"]["ding"];
    int shuibeng = doc["params"]["shuibeng"];
    int paizhao = doc["params"]["paizhao"];
    ...
    char paizhaoStr[16];
    if(paizhao != 0){
      sprintf(paizhaoStr, "paizhao%d", paizhao);
      Serial.println(paizhaoStr);
      }
